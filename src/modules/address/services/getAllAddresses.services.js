...
const getAllAddresses = async (page, limit, filter) => {
    try {
        ...
        const coupons = await ADDRESS_MODEL.find(filterQuery)
            .skip(skip)
            .limit(length)
            .sort(sortQuery)
            .lean();
        const totalResults = await ADDRESS_MODEL.countDocuments(filterQuery);
